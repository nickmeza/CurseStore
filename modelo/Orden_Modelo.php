...
class Orden_Modelo
{
    ...
    {
        return $data = Database::query("SELECT o.*, IF(o.ORD_APPROVAL=1,'Aprobado','Inactivo') AS estado, p.PER_ID ,p.PER_NOMBRE ,p.PER_APELLIDO, p.PER_DIRECCION,p.PER_ESTADO, c.CLI_CREATE FROM orden o, cliente c,usuario u,persona p WHERE c.USR_ID=u.USR_ID AND u.PER_ID=p.PER_ID AND ORD_APPROVAL = 1  AND o.CLI_ID=c.CLI_ID ORDER BY ORD_ID DESC ;", array());
    }
