...
class MetodoPago_Modelo{
    ...
    public static function updateMetodo($metodoP,$id){
        if($metodoP['MET_IMAGEN']!="publico/img/imagen_default.png" ) {
        Database::queryChange("UPDATE metodo_pago set MET_NOMBRE=?,MET_DATOS=?,MET_IMAGEN=?,MET_ESTADO=?, where MET_ID=?",
            array($metodoP["MET_NOMBRE"],$metodoP["MET_DATOS"],$metodoP["MET_IMAGEN"],$metodoP["MET_ESTADO"],$id));
    } else {
        Database::queryChange(
            "UPDATE metodo_pago set MET_ESTADO=?,MET_NOMBRE=?, MET_DATOS=? where MET_ID=?",
    ...
    public static function delete($id){
     ...
     public static function getbystatus(){
            return $data=Database::query("SELECT * FROM metodo_pago where MET_ESTADO = 1 ",array());
        }
 }
