...
class Categoria_Modelo
{
    ...
    public static function insertCategoria($categoria)
    {
        Database::queryChange(
            "INSERT INTO categoria(CAT_NAME,CAT_DESCRIPCION,CAT_ESTADO,CAT_IMAGEN) VALUES (?,?,?,?);",
    ...
    public static function updateCategoria($categoria, $id)
    {
        if ($categoria['CAT_IMAGEN'] != "publico/img/imagen_default.png") {
            Database::queryChange(
                "UPDATE categoria set CAT_NAME=?,CAT_DESCRIPCION=?,CAT_ESTADO=?,CAT_IMAGEN=? where CAT_ID=?",
                array($categoria["CAT_NAME"], $categoria["CAT_DESCRIPCION"], $categoria["CAT_ESTADO"], $categoria['CAT_IMAGEN'], $id)
            );
        } else {
            Database::queryChange(
                "UPDATE categoria set CAT_NAME=?,CAT_DESCRIPCION=?,CAT_ESTADO=? where CAT_ID=?",
    ...
    public static function getByName($buscador)
    {
        ...
        return $data = Database::query("SELECT cu.* FROM categoria c,curso cu where c.CAT_NAME LIKE '%$ggetname%' AND cu.CAT_ID=c.CAT_ID ", array());
    }
